...
- **macOS / Linux**: 双击 `start-chatbot.command`（会自动打开浏览器）
- **Windows**: 双击 `start-chatbot.bat`（会自动打开浏览器）

浏览器打开后，点击左侧边栏的「🛑 停止并退出 / Stop & quit」按钮即可关闭应用，**无需手动 Ctrl+C**。

> 首次运行会自动下载模型权重（取决于选中的档位），如果在中国大陆请确保已设置 `HF_MIRROR=1`（脚本已内置）。
